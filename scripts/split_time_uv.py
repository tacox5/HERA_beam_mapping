...
def new_uvs(uv, time, polarization, folder):
    ...
    name = (
        os.path.basename(folder)[:20]
        + str(time[0]).split(".")[1][:5].ljust(5, "0")
        + os.path.basename(folder)[27:]
    )
...
if __name__ == "__main__":
    n = input(
        "How many segments would you like to split this file into? (Default is 1) "
    )
    n = int(n)
    try:
        folders = sys.argv[1:]
