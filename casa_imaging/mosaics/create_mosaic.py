...
def create_beam_avg(fitsfile):
    ...
    for i in range(101):
        fullbeamavg += hera_beam.data_array[0][0][0][i] / np.max(
            hera_beam.data_array[0][0][0][i]
        )
...
def data_paste_mod(npz):
    ...
    new_beam_count[pixels] += beam_factor_2D(
        coords[:, 0], coords[:, 1], center_ra_dec[0]
    )
...
if __name__ == "__main__":
    try:
        ...
        projected_map = hp.mollview(
            emptymap + 2, norm="log", max=2.05, xsize=4800, return_projected_map=True
        )
